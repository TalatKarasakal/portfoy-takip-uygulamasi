...
class PortfolioService:
    ...
    def calculate_cost_and_pnl(
        transactions: List[Transaction], current_price: float, method: str = "WAC"
    ) -> Dict[str, Any]:
        """
        ...
        if method == "WAC":
            remaining_quantity, average_cost, realized_pnl = (
                PortfolioService._calculate_wac(txs)
            )
        elif method in ("FIFO", "LIFO"):
            remaining_quantity, average_cost, realized_pnl = (
                PortfolioService._calculate_fifo_lifo(txs, method)
            )
        else:
    ...
    @staticmethod
    def _calculate_wac(txs: List[Transaction]) -> Tuple[float, float, float]:
        ...
        for tx in txs:
            ...
            if tx.transaction_type == TransactionType.BUY:
                ...
                if remaining_quantity > 0:
                    ...
                    net_revenue = (float(tx.unit_price) * qty) - (
                        float(tx.commission) + float(tx.tax)
                    )
        ...
        average_cost = (
            (total_invested / remaining_quantity) if remaining_quantity > 0 else 0.0
        )
        return remaining_quantity, average_cost, realized_pnl

    @staticmethod
    def _calculate_fifo_lifo(
        txs: List[Transaction], method: str
        ...
        for tx in txs:
            ...
            if tx.transaction_type == TransactionType.BUY:
                ...
                unit_full_cost = (
                    float(tx.unit_price) * qty + float(tx.commission) + float(tx.tax)
                ) / qty
        ...
        if remaining_quantity > 0:
            total_remaining_cost = sum(
                item["qty"] * item["unit_cost"] for item in inventory
            )

...
class ImportExportService:
    ...
    def export_excel(session: Session, file_path: str):
        ...
        with pd.ExcelWriter(file_path, engine="openpyxl") as writer:
            pd.DataFrame(asset_data).to_excel(
                writer, sheet_name="Varlıklar", index=False
            )
    ...
    def import_excel(session: Session, file_path: str) -> bool:
        ...
        try:
            ...
            for sheet_name, df in dfs.items():
                ...
                if (
                    any("tarih" in c for c in cols)
                    and any("kod" in c for c in cols)
                    and any("tür" in c for c in cols)
                ):
                    success_any = (
                        ImportExportService._process_full_transaction_history(
                            session, df
                        )
                        or success_any
                    )
                ...
                # Senaryo 2: Adet + Maliyet
                    ...
                    any("kod" in c for c in cols)
                    and any("adet" in c for c in cols)
                    and any("maliyet" in c for c in cols)
                ):
                    success_any = (
                ...
                # Kendi "Varlıklar" listemizse veya basit liste ("Fon Kodu", "Fon Adı") ise
                    ...
                    success_any = (
                        ImportExportService._process_assets_only(session, df)
                        or success_any
                    )
    ...
    def _process_full_transaction_history(session: Session, df: pd.DataFrame) -> bool:
        # Pre-load assets into a dictionary to avoid N+1 queries
    ...
    def _process_quantity_cost(session: Session, df: pd.DataFrame) -> bool:
        ...
        for index, row in df.iterrows():
            ...
            tx = Transaction(
                ...
                unit_price=row.get(
                    "Ortalama Maliyet",
                    row.get("maliyet", row.get("ortalama_maliyet", 0)),
                ),
                commission=0,
                tax=0,

...
| PT-015 | **Kontrol Bekliyor.** [GPT-5] Alt paket keşfi ve migration/QSS/font/ikon package-data tamamlandı; izlenen PyInstaller spec, macOS DMG üretimi ve Windows Inno Setup kurucusu eklendi. | `a2a970e` + `146c6e9` | Wheel smoke, macOS PyInstaller `--smoke-test`, DMG bütünlük/bağlı-imaj smoke testi ve CI üç platform paket matrisi. | DMG'yi açıp uygulamayı Applications'a sürükleyin; Windows CI artefaktındaki Setup.exe'yi kurup masaüstü/Başlat menüsü kısayollarını doğrulayın. |
| PT-019 | **Kontrol Bekliyor.** [GPT-5] CI; kalite kontrolleri ve üç platform paket smoke sonrasında macOS DMG ile Windows Setup.exe dosyalarını SHA-256 toplamlarıyla artefakt olarak üretecek şekilde genişletildi. | `a2a970e` + `146c6e9` | Yerelde 142 test, Ruff, MyPy, macOS DMG doğrulaması ve bağlı-imaj smoke testi geçti; Windows kurucusu Windows CI çalıştırıcısında doğrulanacak. | Değişiklikler uzak depoya gönderildikten sonra GitHub Actions'ta validate/package işlerini ve indirilebilir iki kurulum artefaktını kontrol edin. |

...
| PT-015 | **Kontrol Bekliyor.** [GPT-5] Paket kaynakları, macOS DMG ve Windows Inno Setup kurucusuna ek olarak şeffaf master'dan türetilen beyaz/koyu tema simgeleri eklendi; çalışan uygulamanın pencere, Dock ve görev çubuğu simgesi aktif temayla değişiyor. | `a2a970e` + `146c6e9` + `1c0cbb7` | 143 test, tema simgesi piksel/doğrulama testi, GUI smoke, macOS PyInstaller smoke ve DMG bütünlük kontrolü geçti. | Ayarlar'dan açık, koyu ve sistem temasını sırayla seçin; pencere ile Dock/görev çubuğu simgesi arka planının beyaz ve `#0F1115` arasında değiştiğini doğrulayın. |
| PT-019 | **Kontrol Bekliyor.** [GPT-5] CI; kalite kontrolleri ve üç platform paket smoke sonrasında macOS DMG ile Windows Setup.exe dosyalarını SHA-256 toplamlarıyla artefakt olarak üretecek şekilde genişletildi. | `a2a970e` + `146c6e9` + `abe8dfe` | Yerelde 143 test, Ruff, MyPy ve güncel macOS DMG doğrulaması geçti; macOS/Windows/Linux paket job'ları ve Windows kurucu üretimi başarıyla doğrulandı. | GitHub Actions'ta validate/package işlerini ve indirilebilir iki kurulum artefaktını kontrol edin. |
